...
class LlamaDecoder(DecoderBase):
    def __init__(
        self,
        name: str,
        model_id: str,
        ...
        super().__init__(name="LLaMA", batch_size=batch_size, temperature=temperature, max_new_tokens=max_new_tokens)
    ...
    def codegen(
        ...
        return [g["generated_text"] for g in generations], 0, 0  # Returning dummy values for tokens and logprobs as LLaMA does not provide them directly.
...
def make_model(name: str, batch_size: int = 1, temperature: float = 0.8):
    if name == "codegen-2B":
        ...
        return LlamaDecoder(
            name="LLaMA",
            model_id="../../code_models/llama3-1b",
            batch_size=batch_size,
            temperature=temperature,
        )
...
def generate_codes(model, prompt, num_samples=1):
    ...
    while sidx < num_samples:
        ...
        code = outputs[0]
        if model.name == "ChatGPT" or model.name == "LLaMA":
            result = code
        else:
            result = prompt + code
